...
do_kernel_metadata() {
	...
	if [ "$mode" = "config" ]; then
		...
		if [ -n "${elements}" ]; then
			...
			# HACK FIX Must remove relative part from path, because scc doesnt support it.
			# When using kas tool to build, all layers path are set with relative paths.
			sccs_defconfig=$( echo "$sccs_defconfig" | sed 's/\/build\/..//g' )
